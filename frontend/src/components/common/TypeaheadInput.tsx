...
interface TypeaheadInputProps {
  ...
  getInitialOptions?: () => Promise<AutocompleteOption[]>; // For top 5 on focus
  label?: string;
  disabled?: boolean;
  className?: string;
  outlined?: boolean; // Customizable outline
  onAddNew?: () => void;
  addNewLabel?: string;
  sizing?: 'sm' | 'md' | 'lg';
  tabIndex?: number;
...
export default function TypeaheadInput({
  ...
  // Load initial options (top 5) when component mounts
  const loadInitialOptions = useCallback(async () => {
    if (!getInitialOptions || hasLoadedInitial) return;

    setIsLoading(true);
    setError(null);
    try {
      const results = await getInitialOptions();
      setOptions(results.slice(0, 5)); // Ensure we only show top 5
  ...
  }, [getInitialOptions, hasLoadedInitial]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (inputValue && inputValue !== value?.name) {
        // User is typing - search with their input
        debouncedSearch(inputValue);
      } else if (!inputValue) {
        // Input is empty - hide dropdown but keep initial options if loaded
        setIsOpen(false);
        setHasSearched(false);
        // Don't clear options if we have initial options loaded - keep them for focus events
        if (!hasLoadedInitial) {
          setOptions([]);
        }
      }
  ...
  }, [inputValue, debouncedSearch, value?.name, hasLoadedInitial, DEBOUNCE_MS]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    switch (e.key) {
      case 'Tab':
        // Allow tab to select current option or close dropdown
  ...
  // Handle input focus - show top 5 or previous results
  const handleInputFocus = async () => {
    calculateDropdownPosition();

    if (inputValue.trim()) {
      // User has typed something - search for it
      debouncedSearch(inputValue);
    } else {
      // No input - load initial top 5 options
  ...
  const inputClassNames = outlined
    ? 'w-full'
    : 'w-full border-none bg-transparent outline-none focus:ring-0';
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        ...
        {!isLoading && (
          <button
            type="button"
            className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 z-10 pointer-events-none`}
            tabIndex={-1}
          >
                    ...
                    {visibleOptions.map((option, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {option.secondary && (
                            <div className="typeahead-secondary text-[10px] leading-4 text-gray-500 dark:text-gray-400 font-normal">
                              {option.secondary}
                            </div>
                          )}
                        </div>
