...
interface OrderReceiptPrintPreviewProps {
...
export default function OrderReceiptPrintPreview({
  payment,
  order,
  cashierName,
}: OrderReceiptPrintPreviewProps) {
  const parsedAmount = useMemo(() => safeParseFloat(payment.amount), [payment.amount]);
  const balanceDue = useMemo(() => safeParseFloat(order.balance_due), [order.balance_due]);
      ...
      >
        ...
        <div className="grid grid-cols-[18%_54%_11%_17%] text-[11pt]">
          ...
          <div className="row-span-3 py-1 pr-2 font-light leading-tight">
            <div className="font-semibold">{order.customer?.name || 'Valued Customer'}</div>
