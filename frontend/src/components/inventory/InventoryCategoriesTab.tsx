...
import type { InvCategory, InvCategoryListResponse } from '@/types/inventory';
...
export default function InventoryCategoriesTab() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(`/inventory/categories/?${params.toString()}`);
  ...
  const handleRowClick = (category: InvCategory) => {
    setSelectedCategory(category);
    setModalMode('edit');
    setShowModal(true);
  };

  const handleActionClick = (action: string, categoryId: number) => {
  ...
  const handleDelete = async (categoryId: number) => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(err, 'Failed to delete category.');
      setError(errorMessage);
    }
  };
  ...
  const renderActions = (row: InvCategory) => (
    <Dropdown
      label=""
    ...
    >
      ...
      <Dropdown.Item onClick={() => handleActionClick('edit', row.id)}>Edit Category</Dropdown.Item>
