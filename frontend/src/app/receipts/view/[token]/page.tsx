...
export default function PublicReceiptView() {
  ...
  useEffect(() => {
    const fetchReceipt = async () => {
      ...
      try {
        setLoading(true);
        const response = await api.get(`/sales/invoices/receipts/view/${token}/`);
        setReceiptData(response.data);
        setError(null);
      } catch {
        try {
          const response = await api.get(`/sales/orders/receipts/view/${token}/`);
          setReceiptData(response.data);
          setError(null);
        } catch (orderErr) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const downloadEndpoint =
        documentType === 'order'
          ? `/sales/orders/receipts/download/${token}/`
          : `/sales/invoices/receipts/download/${token}/`;
        ...
        <Card className="max-w-md">
          <div className="text-center">
            ...
            <p className="mt-4 text-xs text-gray-400 dark:text-gray-500">
              This receipt link may have expired or is invalid. Please contact the sender for a new
              link.
            </p>
          </div>
        </Card>
